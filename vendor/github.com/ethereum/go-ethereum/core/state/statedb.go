...
func (n *proofList) PutCF(cf string, key []byte, value []byte) error {
...
func New2(root *trie.NewKey, db Database, snaps *snapshot.Tree) (*StateDB, error) {
	var tr Trie
	var err error

	if root == nil {
		// Handle the case where root is nil (use an empty root)
		tr, err = db.OpenTrie(common.Hash{})
	} else {
		tr, err = db.OpenTrieWithNewKey(root)
	}
	...
	if sdb.snaps != nil {
		if sdb.snap = sdb.snaps.Snapshot(common.BytesToHash(root.Bytes())); sdb.snap != nil {
			sdb.snapDestructs = make(map[common.Hash]struct{})
			sdb.snapAccounts = make(map[common.Hash][]byte)
			sdb.snapStorage = make(map[common.Hash]map[common.Hash][]byte)
		}
