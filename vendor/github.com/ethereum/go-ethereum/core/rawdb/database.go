...
func NewRocksDBDatabase2(file string, cache int, handles int, namespace string) (ethdb.Database, error) {
	columnFamilies := []string{
		"default",          // Default column family
		"headers",          // For headers
		"bodies",           // For bodies
		"receipts",         // For receipts
		"tds",              // For total difficulties
		"canonicalHashes",  // For canonical hashes
		"headerNumbers",       // For head block, head header, and head fast block hashes
		"metadatas",
		"stateNode",
		"statePreimage",
	...
	db, err := rocksdb.NewCF(file, cache, handles, namespace, columnFamilies)
	if err != nil {
		return nil, err
	}
	return NewDatabase(db), nil
