...
func NewWithNewKey(key *NewKey, db *Database) (*Trie, error) {
	if db == nil {
		panic("trie.NewWithNewKey called without a database")
	}
	trie := &Trie{
		db: db,
	}
	root := key.Bytes()
	if root != nil {
		rootnode, err := trie.resolveHash2(root[:], nil)
		if err != nil {
			return nil, err
		}
		trie.root = rootnode
...
func (t *Trie) tryGet(origNode node, key []byte, pos int) (value []byte, newnode node, didResolve bool, err error) {
	...
	case *NewKey:
		childKey := n.Bytes()
		childNode := t.db.nodeByNewKey(childKey)
		if err != nil {
			return nil, n, true, err
		}
		// Recursively call tryGet on the retrieved child node
		value, newnode, _, err = t.tryGet(childNode, key, pos)
		return value, newnode, true, err
	default:
...
func (t *Trie) resolveHash2(key []byte, prefix []byte) (node, error) {
	if len(key) != 42 {
		return nil, fmt.Errorf("invalid key length: expected 42 bytes, got %d", len(key))
	}
	if node := t.db.nodeByNewKey(key); node != nil {
		return node, nil
	}
	//return nil, &MissingNodeError{NodeHash: common.BytesToHash(key[10:]), Path: prefix}
	return nil, nil
